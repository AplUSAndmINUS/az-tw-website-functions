  ...
  public async Task<bool> RunTestsAsync()
  {
    ...
    try
    {
      ...
      if (getResult != null)
      {
        ...
        Console.WriteLine($"   - Created: {retrieved.Timestamp}");
        Console.WriteLine($"   - URL: {retrieved.Url}");
      }
      else
      {
      ...
      if (batchGetResult != null && batchGetResult.Any())
      {
        ...
        {
          Console.WriteLine($"   - {item.Filename} ({item.Id})");
        }
      }
      else
      {
    ...
    where T : notnull
{
  ...
  {
    Console.WriteLine($"[INFO] {typeof(T).Name}: Table upsert - Table: {tableName}, Function: {functionName}, PK: {partitionKey}, RK: {rowKey}");
  }

  public void LogTableEntryDelete(string tableName, string functionName, string partitionKey, string rowKey)
  {
    Console.WriteLine($"[INFO] {typeof(T).Name}: Table delete - Table: {tableName}, Function: {functionName}, PK: {partitionKey}, RK: {rowKey}");
  }

  public void LogBlobDownload(string containerName, string functionName, string blobName)
  {
    Console.WriteLine($"[INFO] {typeof(T).Name}: Blob download - Container: {containerName}, Function: {functionName}, Blob: {blobName}");
  }

  public void LogBlobUpload(string containerName, string functionName, string blobName, long size)
  {
    Console.WriteLine($"[INFO] {typeof(T).Name}: Blob upload - Container: {containerName}, Function: {functionName}, Blob: {blobName}, Size: {size}");
  }
}
